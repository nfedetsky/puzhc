...
/**
 Сервис для клонирования заявок по Исправление
 */
@Service
public class CloneRequestByCorrection implements ICloneRequest {
    private final Logger log = LoggerFactory.getLogger(CloneRequestByCorrection.class);

    private final CloneRequestService cloneRequestService;
    @Autowired
    public CloneRequestByCorrection(CloneRequestService cloneRequestService) {
        this.cloneRequestService = cloneRequestService;
    }

    @Override
    public Request cloneRequest(@NotNull Request request) {
        request.setRequestVis(null);
        request.setRequestContract(null);
        request.setRequestModification(null);
        return request;
    }
}