...
@Component(BcAddMinutes.NAME)
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class BcAddMinutes implements JavaDelegate {

    public static final String NAME="bpm_BusinessCalendar_AddMinutes";
    ...
    private final Logger log = LoggerFactory.getLogger(BcAddMinutes.class.getName());

    @Override
    public void execute(DelegateExecution execution) throws FlowableException {

            int aMinutes = Integer.parseInt(minutes.getExpressionText() == null ? "0" : minutes.getExpressionText());
        ...
        if (aMinutes > 0) {
            ...
            time = businessCalendar.plus(time, Duration.ofMinutes(aMinutes));
